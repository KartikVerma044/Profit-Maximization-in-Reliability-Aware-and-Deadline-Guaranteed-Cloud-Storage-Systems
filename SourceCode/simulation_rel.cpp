...
void executeRequest()
{
	...
	for(int i=0;i<servers.size();i++)
	{
		...
		service_cost+=servers[i]->server_reliability*servers[i]->request_queue.size()*servers[i]->maintainence_cost_per_request;
...
int main()
{
	...
	for(double avg_rel=0.1;avg_rel<1;avg_rel+=0.2){
		min_tenant_reliability=avg_rel-0.01;
		max_tenant_reliability=avg_rel+0.01;
		initializeTenants();
