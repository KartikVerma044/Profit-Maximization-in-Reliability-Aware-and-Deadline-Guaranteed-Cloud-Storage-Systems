...
int max_maintainence_cost_per_request = 21;
int min_maintainence_cost_per_request = 20;
...
double min_tenant_deadline = 0.01;

int num_initial_servers = 1000;
int num_data_partitions = 10;
int num_tenants = 100;
...
struct DataPartition{
	...
	DataPartition(){
		...
		charge_per_request = storage_capacity_required*(1+rand()%num_data_partitions);
	}
};
...
struct Tenant{
	...
	Tenant(){
		required_deadline = min_tenant_deadline+(max_tenant_deadline-min_tenant_deadline)*rand()/RAND_MAX;
		required_reliability = min_tenant_reliability+(max_tenant_reliability-min_tenant_reliability)*rand()/RAND_MAX;
...
struct Server{
	...
	Server(){
		...
		initialization_cost=service_rate*5+storage_capacity*10;
		storage_cost_per_unit=min_storage_cost_per_unit+rand()%(max_storage_cost_per_unit-min_storage_cost_per_unit);
		maintainence_cost_per_request=min_maintainence_cost_per_request+rand()%(max_maintainence_cost_per_request-min_maintainence_cost_per_request);
	}
};
	...
	for(int i=0; i<servers.size(); i++){
		if(servers[i]->available_service_capacity()>0 && servers[i]->server_reliability >= tenants[incoming_request->tenant]->required_reliability && servers[i]->available_storage_capacity()>=data_partitions[data_partition_id]->storage_capacity_required){
            if(servers[i]->tenants.find(incoming_request->tenant)!=servers[i]->tenants.end()){
                ...
                int new_deadline_guaranteed_request_arrival_rate=min(servers[i]->deadline_guaranteed_request_arrival_rate, servers[i]->service_rate-servers[i]->service_rate*tenants[incoming_request->tenant]->tenant_f1/(1-exp(-(servers[i]->service_rate+servers[i]->unsuccessful_storage)*tenants[incoming_request->tenant]->required_deadline))-servers[i]->unsuccessful_storage*tenants[incoming_request->tenant]->tenant_f1);
	...
	for(int i=0;i<servers.size();i++)
	{
		...
		for(int j=0; j<servers[i]->request_queue.size();j++){
			if(servers[i]->server_reliability>1.0*rand()/RAND_MAX){
				...
				// profit+=data_partition_cost*(1+exp(reliability))/deadline;
				// profit+=data_partition_cost*(1+exp(reliability))*(1+exp(-deadline));
				profit+=data_partition_cost*(1+exp(reliability))/(1+exp(deadline));
				// profit+=data_partition_cost*(1+exp(reliability))*(1-deadline);
				// profit+=data_partition_cost/((1-reliability)*deadline);
...
int main()
{
	...
	for(double avg_dl=0.1;avg_dl<1;avg_dl+=0.2){
		min_tenant_deadline=avg_dl-0.01;
		max_tenant_deadline=avg_dl+0.01;
		initializeTenants();
		for(int algo=1;algo<9;algo++){
			...
			for(int loop=0;loop<num_intervals;loop++)
			{
				...
				// requestGeneration();
				for(int i=0;i<num_requests;i++) incoming_requests.push_back(incoming_requests_backup[requests_used++]);
				// Request Allotment
				profit=0;
			...
			// cout<<"init - "<<init_cost/(num_requests*num_intervals)<<" svc - "<<service_cost/(num_requests*num_intervals)<<" stc - "<<storage_cost/(num_requests*num_intervals)<<endl;
