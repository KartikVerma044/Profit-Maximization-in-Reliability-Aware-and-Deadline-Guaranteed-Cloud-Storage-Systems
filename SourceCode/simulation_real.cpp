...
// string trace_path = "C:\\Users\\Vin-It\\Downloads\\BTP\\Dataset\\Spotify\\trace.csv";
// char seperator = ' ';

// netflix
int num_data_partitions = 250;
int num_tenants = 3;
int num_intervals = 2500;
int num_requests = 10000;
string trace_path = "C:\\Users\\Vin-It\\Downloads\\BTP\\Dataset\\Netflix\\trace.csv";
char seperator = ',';
...
struct Request{
	...
	Request(int dataid, int timestamp, int userid=rand()){
		int ran = userid%num_tenants;
		if(num_tenants==3){
			if(ran==0) tenant=0;
			else if(ran==1) tenant=1;
			else tenant=2;
		}
		else{
			if(ran==0) tenant=0;
